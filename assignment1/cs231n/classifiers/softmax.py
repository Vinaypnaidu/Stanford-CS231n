...
def softmax_loss_naive(W, X, y, reg):
    ...
    passnum_classes = W.shape[1]
    num_train = X.shape[0]
    for i in range(num_train):
        
        # forward pass using staged computation
        scores = X[i].dot(W)
        scores -= scores.max()
        num = np.exp(scores)
        den = np.sum(num)
        ...
        # backprop using staged computation
        dprobs = np.zeros(probs.shape)
        dprobs[y[i]] = -1.0 / probs[y[i]]
        dinvden = num.dot(dprobs)
        dnum = invden * dprobs
        dden = (-1.0 / (den ** 2)) * dinvden
        dnum += np.ones(num.shape) * dden
        dscores = np.reshape(num * dnum, (1, -1))
