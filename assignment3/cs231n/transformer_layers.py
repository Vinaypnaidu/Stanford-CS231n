...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # divide the matrices into multiple heads
        query = self.query(query)
        key = self.key(key)
        value = self.value(value)
        q = query.reshape(N, S, H, E // H).permute(0, 2, 1, 3)
        k = key.reshape(N, T, H, E // H).permute(0, 2, 1, 3)
        v = value.reshape(N, T, H, E // H).permute(0, 2, 1, 3)

        factor = 1 / (math.sqrt(E / H))
        y = torch.matmul(q, k.permute(0, 1, 3, 2)) * factor
        ...
        y = torch.matmul(y, v)
        output = self.proj(y.permute(0, 2, 1, 3).reshape(N, S, E))
