...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    h_inter = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
    next_h = np.tanh(h_inter)
    cache = (h_inter, prev_h, x, Wh, Wx)
...
def rnn_step_backward(dnext_h, cache):
    ...
    h_inter, prev_h, x, Wh, Wx = cache
    dh_inter = (1.0 - np.square(np.tanh(h_inter))) * dnext_h
    db = np.sum(dh_inter, axis=0)
    dWh = np.dot(prev_h.T, dh_inter)
    dWx = np.dot(x.T, dh_inter)
    dprev_h = np.dot(dh_inter, Wh.T)
    dx = np.dot(dh_inter, Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        h0, cache_step = rnn_step_forward(x[:, t, :], h0, Wx, Wh, b)
        h[:, t, :] = h0
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dnext_h += dh[:, t, :] 
        dx_t, dnext_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, cache[t])
        dx[:, t, :] = dx_t
        dh0 = dnext_h
        dWx += dWx_t
        dWh += dWh_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = next_c, prev_c, i, f, o, g, A, x, prev_h, Wx, Wh, H
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    next_c, prev_c, i, f, o, g, A, x, prev_h, Wx, Wh, H = cache
    dA = np.zeros(A.shape)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        h0, c0, cache_step = lstm_step_forward(x[:, t, :], h0, c0, Wx, Wh, b)
        h[:, t, :] = h0
        cache.append(cache_step)
